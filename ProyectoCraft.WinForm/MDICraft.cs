...
using SCCMultimodal.Ventas.Pricing;
...
namespace ProyectoCraft.WinForm {
    ...
    public partial class MDICraft : Form {
        ...
        private void navBarItem5_LinkClicked(object sender, NavBarLinkEventArgs e) {
            var timer = System.Diagnostics.Stopwatch.StartNew();
            var form = FrmSolicitarTarifa.Instancia;
            form.MdiParent = this;
            form.Show();
            ClsLogPerformance.Save(new LogPerformance(Base.Usuario.UsuarioConectado.Usuario, timer.Elapsed.TotalSeconds));
        }
        ...
        private void navBarItem6_LinkClicked(object sender, NavBarLinkEventArgs e) {
            var timer = System.Diagnostics.Stopwatch.StartNew();
            var form = FrmListarCotizaciones.Instancia;
            form.MdiParent = this;
            form.Text = "Mis Cotizaciones";
            form.Show();
            ClsLogPerformance.Save(new LogPerformance(Base.Usuario.UsuarioConectado.Usuario, timer.Elapsed.TotalSeconds));
        }

        private void menuPricing_LinkClicked(object sender, NavBarLinkEventArgs e) {
            var timer = System.Diagnostics.Stopwatch.StartNew();
            var form = FrmListarTarifas.Instancia;

            form.MenuIngresarTarifa.Visible = true;
            form.menuNuevaTarifa.Visible = false;
            form.MenuVolverCotizar.Visible = false;
            /*form.MenuRechazadoCliente.Visible = false;
            form.menuCerradoSinFeedback.Visible = form.menuGanado.Visible = form.MenuCerradoRechazado.Visible =
                           form.MenuRechazadoCliente.Visible = false;
            */
             form.Text = "Mis Tarifas";
            form.MdiParent = this;
            form.Show();
            ClsLogPerformance.Save(new LogPerformance(Base.Usuario.UsuarioConectado.Usuario, timer.Elapsed.TotalSeconds));
        }
