...
namespace ProyectoCraft.WinForm.Paperless.Asignacion
{
    public partial class frmPaperlessAsignacion : Form
    {
        ...
        {
            txtAperturaNavieras.Properties.DisplayFormat.Format = new CultureInfo("es-CL");
            txtAperturaNavieras.Properties.Mask.Culture = new CultureInfo("es-CL");
            txtAperturaNavieras.Properties.DisplayFormat.FormatType = FormatType.DateTime;
            txtAperturaNavieras.Properties.DisplayFormat.Format = new CultureInfo("es-CL").DateTimeFormat;
            txtAperturaNavieras.Properties.DisplayFormat.FormatString = "d";


            txtFechaETA.Properties.DisplayFormat.Format = new CultureInfo("es-CL");
            txtFechaETA.Properties.Mask.Culture = new CultureInfo("es-CL");
            txtFechaETA.Properties.DisplayFormat.FormatType = FormatType.DateTime;
            txtFechaETA.Properties.DisplayFormat.Format = new CultureInfo("es-CL").DateTimeFormat;
            txtFechaETA.Properties.DisplayFormat.FormatString = "d";

            txtfechaMasterConfirmado.Properties.DisplayFormat.Format = new CultureInfo("es-CL");
            txtfechaMasterConfirmado.Properties.Mask.Culture = new CultureInfo("es-CL");
            txtfechaMasterConfirmado.Properties.DisplayFormat.FormatType = FormatType.DateTime;
            txtfechaMasterConfirmado.Properties.DisplayFormat.Format = new CultureInfo("es-CL").DateTimeFormat;
            txtfechaMasterConfirmado.Properties.DisplayFormat.FormatString = "d";

            txtPlazoEmbarcadores.Properties.DisplayFormat.Format = new CultureInfo("es-CL");
            txtPlazoEmbarcadores.Properties.Mask.Culture = new CultureInfo("es-CL");
            txtPlazoEmbarcadores.Properties.DisplayFormat.FormatType = FormatType.DateTime;
            txtPlazoEmbarcadores.Properties.DisplayFormat.Format = new CultureInfo("es-CL").DateTimeFormat;
            txtPlazoEmbarcadores.Properties.DisplayFormat.FormatString = "d";
        ...
        private bool ValidarPaso2()
        {
            ...
            {
                var conf = Base.Configuracion.Configuracion.Instance();
                var op = conf.GetValue("DiasAntesFechaApertura"); //puede retornar un true, false o null
                if (op.HasValue && op.Value.Equals(true))
                {
                    ...
                    {
                        DiasAntesFechaApertura =
                            Convert.ToInt16(
                                System.Configuration.ConfigurationSettings.AppSettings.Get("DiasAntesFechaApertura"));
                        if (fApertura > fETA.AddDays(-DiasAntesFechaApertura))
                        {
                            if (
                                MessageBox.Show(
                                    "Esta ingresando una Fecha de Apertura menor a " + DiasAntesFechaApertura +
                                    " días de anterioridad que la Fecha ETA.\n ¿Esta seguro de ingresar esta fecha? \n",
                                    "Paperless", MessageBoxButtons.YesNo, MessageBoxIcon.Question,
                                    MessageBoxDefaultButton.Button2) == DialogResult.No)
                            {
                                txtAperturaNavieras.Text = "";
                                //txtAperturaNavieras.Focus();
                                valida = false;
                                dxErrorProvider1.SetError(txtAperturaNavieras, "Debe seleccionar fecha apertua navieras",
                                                          ErrorType.Critical);
                            }
                        }
                    }
            ...
            if (opcion.HasValue && opcion.Value.Equals(true))
            {
                if (txtFechaMaximaVinculacion.Text.Length.Equals(0))
                {
                    MessageBox.Show("Debe completar Fecha Máxima de Vinculación", "Paperless", MessageBoxButtons.OK,
            ...
            if (!string.IsNullOrEmpty(txtAperturaNavieras.Text))
            {
                ...
                if (valorconfiguracion.HasValue && valorconfiguracion.Value.Equals(true))
                    if (!string.IsNullOrEmpty(txtAperturaNavieras.Text) && !string.IsNullOrEmpty(txtFechaETA.Text))
                    {
                        ...
                        if (time.TotalDays < 4)
                        {
                            MessageBox.Show(@"La fecha de Transmisión debe ser menor a 4 días que la fecha ETA.", "Paperless", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            valida = false;
                        }
        ...
        private void LimpiarFormulario()
        {
            txtAgente.Text = "";
            txtAperturaNavieras.Text = "";
            txtAperturaNavieras.EditValue = null;
            txtFechaETA.Text = "";
            txtFechaMaster.Text = "";
            txtNave.Text = "";
            txtNaviera.Text = "";
        ...
        private void txtFechaETA_EditValueChanged(object sender, EventArgs e)
        {
            ...
            if (valorconfiguracion.HasValue && valorconfiguracion.Value.Equals(true))
                if (string.IsNullOrEmpty(txtAperturaNavieras.Text) && !string.IsNullOrEmpty(txtFechaETA.Text))
                {
                    //txtAperturaNavieras.Properties.Mask.UseMaskAsDisplayFormat = true;
                    //txtAperturaNavieras.Properties.Mask.MaskType = MaskType.DateTime;                    
                    //txtAperturaNavieras.Properties.Mask.Culture
                    //txtAperturaNavieras.Properties.DisplayFormat.FormatType = FormatType.DateTime;
                    //txtAperturaNavieras.Properties.DisplayFormat.FormatString = "dd/MM/yyyy";

                    txtAperturaNavieras.Text = Convert.ToDateTime(txtFechaETA.Text).AddDays(-4).ToString("MM/dd/yyyy");
                    //txtAperturaNavieras.Properties.Mask.MaskType = DevExpress.XtraEditors.Mask.MaskType.DateTime;
                    txtAperturaNavieras.Properties.Mask.EditMask = "d";//@"dd\/MM\/yyyy";
                    //txtAperturaNavieras.Properties.DisplayFormat.FormatType = DevExpress.Utils.FormatType.DateTime;
                    txtAperturaNavieras.Properties.DisplayFormat.FormatString = "d";// @"dd\/MM\/yyyy";
                    txtAperturaNavieras.Properties.Mask.UseMaskAsDisplayFormat = true;
                }
                else
