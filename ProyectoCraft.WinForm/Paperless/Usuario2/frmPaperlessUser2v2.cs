...
namespace ProyectoCraft.WinForm.Paperless.Usuario2
{
    ...
    public partial class frmPaperlessUser2v2 : Form, IFrmPaperlessUser2
    {
        ...
        private void frmPaperlessUser2_Load(object sender, EventArgs e)
        {
            ...
            string valorBusqueda = conf.GetValueString("Pless_User2_Contactar_Embarcador_String");
            if (!string.IsNullOrEmpty(valorBusqueda))
                labelControl2.Text = valorBusqueda;

            string valorBusqueda2 = conf.GetValueString("Pless_User2_GridView_Embarcador_String");
            if (!string.IsNullOrEmpty(valorBusqueda2))
                gridView3.Columns.View.Columns[0].Caption = valorBusqueda2;

            //            		gridView3.Columns.View.Columns[0].Caption	"Embarcador"	string
        ...
        private void CargarP3AperturaEmbarcadores()
        {
            ...
            foreach (var paperlessUsuario1HousesBl in houses)
            {
                ...
                if (paperlessUsuario1HousesBl.TipoReciboAperturaEmbarcador.Equals(Enums.PaperlessTipoReciboAperturaEmbarcador.NoDefinido))
                    ...
                    else if (paperlessUsuario1HousesBl.TipoReciboAperturaEmbarcador.Equals(Enums.PaperlessTipoReciboAperturaEmbarcador.NoDefinido))
                        paperlessUsuario1HousesBl.TipoReciboAperturaEmbarcador = Enums.PaperlessTipoReciboAperturaEmbarcador.Mail;
            }
        ...
        private void MarcarCambioEstadoPasoChile(object sender, EventArgs e)
        {
            ...
            if (paso.Paso.NumPaso == 1)//|| paso.Paso.NumPaso == 2 || paso.Paso.NumPaso == 3)//mejora peru
            {
                paso.Estado = false;
                CargarPasos();
                return;
            ...
            ResultadoTransaccion resultado1 = LogicaNegocios.Paperless.Paperless.Usuario2ActualizaPaso(PaperlessAsignacionActual, paso);


        }
        private void TerminaProcesoUsuario2(PaperlessPasosEstado paso)
        {
            ...
            if (paso != null)
            {
                ...
                else
                {
                    ...
                    {
                        Cursor.Current = Cursors.Default;
                        MessageBox.Show("Error al enviar email de término de proceso. \n" + resultado.Descripcion, "Paperless",
                                        MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                    else
                    {
        ...
        private bool ValidarAperturaEmbarcadores()
        {
            ...
            foreach (var house in houses)
            {
                ...
                {
                    valida = false;
                    MessageBox.Show("Para el Embarcador " + house.Cliente.NombreCompañia + " selección de recibo.",
                                    "Paperless", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    break;
                }
                if (ValidaTipoReciboAperturaEmbarcador(house).Equals(true) && house.Observacion.Equals(""))
                {
                    MessageBox.Show("Debe ingresar una observación en " + house.Cliente.NombreCompañia, "Paperless", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    valida = false;
                    break;
                }
            }
        ...
        private void btnGuardarConfMaster_Click_1(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(txtfechaMasterConfirmado.Text))
                ...
                PaperlessAsignacionActual.FechaMasterConfirmado=Convert.ToDateTime("01-01-9999");
            ...
            PaperlessPasosEstado pasoSeleccionado = ObtenerPasoSelccionadoDesdeGrilla();
            pasoSeleccionado.Estado = true;
            if (PaperlessAsignacionActual.ChkMasterConfirmado.Equals(true))
            {
                ResultadoTransaccion resultado1 = new ResultadoTransaccion();
                resultado1 = LogicaNegocios.Paperless.Paperless.GuardarConfirmarMaster(PaperlessAsignacionActual);

                ResultadoTransaccion resultado;
                resultado = LogicaNegocios.Paperless.Paperless.Usuario2CambiarEstadoPaso(pasoSeleccionado);
                CargarPasos();
            ...
            {
                MessageBox.Show("Debe seleccionar Master Confirmado. \n" , "Paperless",
                                           MessageBoxButtons.OK, MessageBoxIcon.Error);
            }

            
        }
        public void ConfirmarMaster(PaperlessPasosEstado paso)
        {
            _pasoEstadoActual = paso;
            PnlConfMaster.Visible = true;
            var asignacion = LogicaNegocios.Paperless.Paperless.ObtenerAsignacionPorId(PaperlessAsignacionActual.Id);
            txtCourier.Text = asignacion.TxtCourier;
            chkConfirmacionMaster.Checked = asignacion.ChkMasterConfirmado;
            if (asignacion.FechaMasterConfirmado != null)
                txtfechaMasterConfirmado.Text = asignacion.FechaMasterConfirmado.ToString();
            radioCourierDestino.SelectedIndex = asignacion.ChkCourier ? 0 : 1;
        ...
        private void btnReenviarCorreoTermino_Click_1(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    MessageBox.Show("Se ha renviado el mail de término de proceso.", "Paperless", MessageBoxButtons.OK,
                                    MessageBoxIcon.Information);
        ...
        private void btnP3GuardarAperturaEmbarcadores_Click_1(object sender, EventArgs e)
        {
            long IdAsignacion = 0;
            try
            {
